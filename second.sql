...
-- create a table by the name of courses having feild name "CourseID", "CourseName", "Course_duration_months", "CourseFee"
CREATE TABLE Course(
...
-- write a command to insert course details into table name course
INSERT INTO Course(CourseName, CourseDuration_Months, CourseFee) Values("The Complete Excel Mastery Course",3,1499);
INSERT INTO Course(CourseName, CourseDuration_Months, CourseFee) Values("DSA For Interview Preparation",3,999);
INSERT INTO Course(CourseName, CourseDuration_Months, CourseFee) Values("SQL Bootcamp",1,1199);
...
SELECT COUNT(Learner_Id) AS num_of_learners_SQL
FROM Learners 
WHERE SelectedCourses=3 ;
-- it will only not null values so mentioned only primary id in it
...
-- 5. Count the number of learners enrolled in the 21 Jan
SELECT COUNT(*) as num_learners_jan FROM Learners
